...
void init_input_with_cpu(const int &sort_param) {
    ...
    // printf("pre order time = %f, ", (iTime6 - iTime1) * 1000.);
}
...
void solve_gridding(const char *infile, const char *tarfile, const char *outfile, const char *sortfile, const int& param, const int &bDim) {
    ...
    double iTime2 = cpuSecond();
    double iElaps = (iTime2 - iTime1) * 1000.;
    // printf("pre_order time=%f, ", iElaps);   
    printf("%f, ", iElaps);  
    // Alloc data for GPU.
    data_alloc();
    ...
    double iTime5 = cpuSecond();
    iElaps = (iTime5 - iTime1) * 1000.;
    // printf("solving_gridding time=%f, ", iElaps);
    printf("%f\n ", iElaps);  
}
