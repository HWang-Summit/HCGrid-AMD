...
/***----------------------------------prepare_grid_kernel-------------------------------***/
/**
 * @brief   Set the gridding kernel type & healpix lookup table.
 * @param   kernel_params:  Set the kernel parameters for the chosen type
 * @param   sphere_radius:  Kernel sphere radius.
 * @param   hpx_max_res:    Maximum acceptable HPX resolution
 * */
...
void read_output_map(const char *infile){
    ...
    // the block warp num of one ring.
    h_GMaps.block_warp_num = (h_zyx[1] - 1) / (32 * h_GMaps.factor) + 1;    // Determined by the scope of RA.
...
int set_WCS(){
    ...
    // Get the dimension
    uint32_t z = h_zyx[0];
    uint32_t y = h_zyx[1];      //  RA
    uint32_t x = h_zyx[2];      //  Dec
