...
void solve_gridding(const char *infile, const char *tarfile, const char *outfile, const char *sortfile, const int& param, const int &bDim, int argc, char **argv) {
    ...
    dim3 block(bDim);
    dim3 grid((h_GMaps.block_warp_num * h_zyx[1] - 1) / (block.x / 64) + 1);  //32-->64
    ...
    #pragma omp parallel
    {
        ...
        pre_order_data(i);
        HANDLE_ERROR(hipMemcpyAsync((double*)((char*)d_data+i*pitch_d), h_data[i], sizeof(double)*data_shape, hipMemcpyHostToDevice, stream[i]));
        HANDLE_ERROR(hipMemcpyAsync((double*)((char*)d_datacube+i*pitch_r), h_datacube[i], sizeof(double)*num, hipMemcpyHostToDevice, stream[i]));
        HANDLE_ERROR(hipMemcpyAsync((double*)((char*)d_weightscube+i*pitch_r), h_weightscube[i], sizeof(double)*num, hipMemcpyHostToDevice,stream[i]));
        hipLaunchKernelGGL(hcgrid, dim3(grid), dim3(block ), 0, stream[i], d_lons, d_lats, (double*)((char*)d_data+i*pitch_d), d_weights, d_xwcs, d_ywcs, (double*)((char*)d_datacube+i*pitch_r), (double*)((char*)d_weightscube+i*pitch_r), d_start_ring, d_hpx_idx);
        data_d2h(i, i);
    }
